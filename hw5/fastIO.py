...
codon_table = {'TTT': 'F', 'TTC': 'F', 'TTA': 'L', 'TTG': 'L', 'CTT': 'L',
'CTC': 'L', 'CTA': 'L', 'CTG': 'L', 'ATT': 'I', 'ATC': 'I', 'ATA': 'I',
'ATG': 'M', 'GTT': 'V', 'GTC': 'V', 'GTA': 'V', 'GTG': 'V', 'TCT': 'S',
'TCC': 'S', 'TCA': 'S', 'TCG': 'S', 'CCT': 'P', 'CCC': 'P', 'CCA': 'P',
'CCG': 'P', 'ACT': 'T', 'ACC': 'T', 'ACA': 'T', 'ACG': 'T', 'GCT': 'A',
'GCC': 'A', 'GCA': 'A', 'GCG': 'A', 'TAT': 'Y', 'TAC': 'Y', 'TAA': '*',
'TAG': '*', 'CAT': 'H', 'CAC': 'H', 'CAA': 'Q', 'CAG': 'Q', 'AAT': 'N',
'AAC': 'N', 'AAA': 'K', 'AAG': 'K', 'GAT': 'D', 'GAC': 'D', 'GAA': 'E',
'GAG': 'E', 'TGT': 'C', 'TGC': 'C', 'TGA': '*', 'TGG': 'W', 'CGT': 'R',
...
def read_codon(table, codon_table=codon_table, singlebase=False, AA=True):
    """
    ...
    for row in table:
        ...
        if len(row) == 5:
            if AA:
                if singlebase:
                    aaKey = codon_table[row[1]]
                else:
                    aaKey = row[0]
                if aaKey not in codon_dic_dic:
                    codon_dic_dic[aaKey] = {row[1]: float(row[2])}
                else:
                    codon_dic_dic[aaKey][row[1]] = float(row[2])
            else:
                codon_dic_dic[row[1]] = float(row[2])


    return codon_dic_dic
