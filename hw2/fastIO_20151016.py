...
def read_fasta(fainput, alphabet='ABCDEFGHIJKLMNOPQRSTUVWXYZ', quality=False):
    """
...
def read_fastq(fqinput, offset):
    ...
    # step = 1 scans for seqID and com line
    ...
    for line in fqinput:
        ...
        if step == 3:
            ...
            if len(seq) == len(qual):
                ...
                continue
    if seqID is not None:
        # Parses final sequence of final
        if len(qual) > 0:
            qual = [ord(char) - offset for char in qual]
        sep = None
